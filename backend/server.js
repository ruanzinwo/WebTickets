...
function filtroRelatorio(tipo) {
  if (tipo === "diario") {
    return "WHERE DATE(data_emissao) = CURDATE()";
  }

  if (tipo === "mensal") {
    return "WHERE YEAR(data_emissao) = YEAR(CURDATE()) AND MONTH(data_emissao) = MONTH(CURDATE())";
  }
...
app.get("/", (req, res) => {
  res.json({
    mensagem: "API WebTickets funcionando"
  });
});
  ...
  try {
    ...
    if (!["SP", "SG", "SE"].includes(tipo)) {
      return res.status(400).json({
        erro: "Tipo inválido. Use SP, SG ou SE."
      });
    }

    if (!dentroDoExpediente() && !ignorarExpediente) {
      return res.status(403).json({
        erro: "Fora do expediente. Atendimento das 07h às 17h."
      });
    }
...
app.get("/senhas/aguardando", async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT tipo, COUNT(*) quantidade FROM senhas WHERE status='AGUARDANDO' GROUP BY tipo"
    );
    ...
    rows.forEach((r) => {
      fila[r.tipo] = Number(r.quantidade);
    });
  ...
  try {
    ...
    if (!senha) {
      return res.status(404).json({
        mensagem: "Não há senhas aguardando."
      });
    }

    const deveDescartar = Math.random() <= 0.05;

    if (deveDescartar) {
      await pool.query(
        "UPDATE senhas SET status='DESCARTADA' WHERE id=?",
        [senha.id]
      );

      return res.json({
        mensagem: "Senha descartada por não comparecimento",
        senha: senha.codigo,
        tipo: senha.tipo,
        descartada: true
      });
...
app.get("/painel", async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT codigo, tipo, guiche, data_atendimento FROM senhas WHERE status='ATENDIDA' ORDER BY data_atendimento DESC LIMIT 5"
    );
    ...
    res.status(500).json({
      erro: "Erro ao buscar painel"
    });
  }
});

app.get("/relatorios/:tipo", async (req, res) => {
  try {
    const tipo = req.params.tipo || "geral";

    if (!["geral", "diario", "mensal", "resumo"].includes(tipo)) {
      return res.status(400).json({
        erro: "Tipo de relatório inválido."
      });
    }

    const filtro = tipo === "resumo" ? "" : filtroRelatorio(tipo);
    ...
    const [detalhes] = await pool.query(`
      SELECT codigo, tipo, data_emissao, data_atendimento, guiche, status, tempo_atendimento
      FROM senhas
      ${filtro}
      ORDER BY id DESC
      LIMIT 300
    ...
    res.json({
      tipoRelatorio: tipo === "resumo" ? "geral" : tipo,
      geral: geral[0],
      porTipo,
      detalhes
    });
...
app.post("/expediente/encerrar", async (req, res) => {
  try {
    const [result] = await pool.query(
      "UPDATE senhas SET status='DESCARTADA' WHERE status='AGUARDANDO'"
    );

    res.json({
      mensagem: "Expediente encerrado. Senhas aguardando descartadas.",
      descartadas: result.affectedRows
    });
  } catch (error) {
    console.error(error);

    res.status(500).json({
      erro: "Erro ao encerrar expediente"
    });
  }
});
...
app.delete("/dev/limpar", async (req, res) => {
  try {
    ...
    res.json({
      mensagem: "Dados apagados para novo teste."
    });
  } catch (error) {
    console.error(error);

    res.status(500).json({
      erro: "Erro ao limpar dados"
    });
  }
});
